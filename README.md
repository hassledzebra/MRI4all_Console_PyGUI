...
- **2D image reconstruction**: k-space → image reusing the console's own recon
  code (`reconstruct.py` loads `mri4all_console/recon/recon_utils/imaging.py`),
  with **k-space apodization** (`kFilter`) and **denoising** (`image_filters`)
  also reused from the console. An **Image** tab displays it. Demo reconstructs a
  synthetic phantom; real imaging needs the gradient board.
- **Averaging (NSA)** for SNR; **Save data** (raw / image / k-space `.npy` + JSON)
  and **DICOM export** (pydicom, console pattern) to `data/`.
